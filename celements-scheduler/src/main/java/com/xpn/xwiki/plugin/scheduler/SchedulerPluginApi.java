 ...
 * library.
 * <p/>
 * Jobs are represented by {@link com.xpn.xwiki.api.Object} XObjects, instances of the
 * {@link SchedulerPlugin#XWIKI_JOB_CLASS} XClass. These XObjects do store a job name, the
 * implementation class name of
 * the job to be executed, the cron expression to precise when the job should be fired, and possibly
 * a groovy script
 * with the job's program.
 * <p/>
 * The plugin offers a {@link GroovyJob} Groovy Job wrapper to execute groovy scripts
 * (typically for use inside the Wiki), but can also be used with any Java class implementing
 * {@link org.quartz.Job}
 *
 * @version $Id$
 */
public class SchedulerPluginApi extends PluginApi<SchedulerPlugin> {
   ...
   *
   * @param object
   *          the XObject job to give the state of
   ...
   * com.xpn.xwiki.api.Object without that
   * the current user needs programming rights (as in com.xpn.xwiki.api.Object#getXWikiObject(). The
   * function is used
   * internally by this api class and allows wiki users to call methods from the scheduler without
   * having programming
   ...
   *
   * @param document
   *          the document holding the XObjects Jobs to be scheduled
   * @return true on success, false on failure.
   */
  ...
  /**
   * Pause the given XObject job by pausing all of its current triggers. Can be called the same way
   * as
   * {@link #scheduleJob(Object)}
   *
   * @param object
   *          the wrapped XObject Job to be paused
   * @return true on success, false on failure.
   */
   ...
   *
   * @param object
   *          the wrapped XObject Job to be triggered
   * @return true on success, false on failure.
   */
   ...
   *
   * @param object
   *          the BaseObject Job to be triggered
   * @return true on success, false on failure.
   */
  ...
  /**
   * Give, for a XObject job in a {@link JobState#STATE_NORMAL} state, the previous date at which
   * the job has been
   * executed, the fire time is not computed from the CRON expression, this method will return null
   * if the .
   *
   * @param object
   *          the wrapped XObject for which to give the fire time
   ...
   *
   * @param object
   *          the BaseObject for which to give the fire time
   * @return the date the job has been executed
   */
  ...
  /**
   * Give, for a XObject job in a {@link JobState#STATE_NORMAL} state, the next date at which the
   * job will be
   * executed, according to its cron expression. Errors are returned in the context map. Can be
   * called for example:
   ...
   *
   * @param object
   *          the wrapped XObject for which to give the fire date
